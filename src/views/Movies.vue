<template>
  <v-container>
    ...
    <v-row class="mb-3">
      <v-col>
        ...
        <v-menu v-model="showCalendar" :close-on-content-click="false" :nudge-right="40" transition="scale-transition" offset-y min-width="auto">
          ...
          <v-date-picker v-model="selectedYear" @input="showCalendar = false" @change="sortByYear" :allowed-dates="allowedDates" color="var(--primary-color)" reactive type="date">
            <v-spacer></v-spacer>
    ...
    <!-- Movies List -->
    <v-row>
      <v-col cols="12" sm="6" md="4" lg="3" v-for="movie in displayedMovies" :key="movie.id">
    ...
    <v-row>
      <v-col class="text-center">
        <v-btn color="var(--accent-color-light)" rounded dark @click="loadMoreMovies" :disabled="loading">Load More</v-btn>
      </v-col>
    </v-row>
  </v-container>
</template>
...
export default {
  ...
  methods: {
    ...
    sortByYear() {
      ...
      axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${process.env.VUE_APP_TMDB_API_KEY}&language=en-US&sort_by=release_date.asc&primary_release_year=${year}`)
        .then(response => {
    ...
    sortMovies() {
      if (this.sortType === "alpha") {
        this.displayedMovies.sort((a, b) =>
          this.isFlipped ? b.title.localeCompare(a.title) : a.title.localeCompare(b.title)
        );
      }
    },
    allowedDates(val) {
