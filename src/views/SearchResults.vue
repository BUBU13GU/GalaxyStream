<template>
  <v-container>
    ...
    <p v-if="movieSearchResults.length === 0 && seriesSearchResults.length === 0">No results found.</p>
  </v-container>
</template>
      ...
      if (this.query) {
        try {
          ...
          const seriesResponse = await axios.get(
            `https://api.themoviedb.org/3/search/tv?api_key=${process.env.VUE_APP_TMDB_API_KEY}&query=${encodeURIComponent(this.query)}`
          );
